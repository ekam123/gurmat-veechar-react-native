...
import { searchCachedItems, CachedFolder } from '@/services/database';
...
export default function BrowseScreen() {
  ...
  const handleResultPress = (item: CachedFolder) => {
    if (item.itemType === 'folder') {
      ...
      router.push(`/browse/folder/${encodeURIComponent(item.parentPath)}` as any);
    }
    setSearchQuery('');
    setSearchResults([]);
  };
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 40, backgroundColor: theme.surface }]}>
        <View style={styles.headerTop}>
          <Text style={[styles.title, { color: theme.text }]}>Gurmat Veechar</Text>
            ...
            {searchResults.map((item) => (
              ...
              >
                <View style={[styles.resultIcon, { backgroundColor: theme.primary + '20' }]}>
                  <Ionicons
                    name={item.itemType === 'folder' ? 'folder' : 'musical-note'}
                    size={18}
                    color={theme.primary}
                  />
                </View>
                <View style={styles.resultText}>
                  ...
                  <Text style={[styles.resultPath, { color: theme.textSecondary }]} numberOfLines={1}>
