import { AUDIO_BASE_URL, BASE_URL } from "@/utils/constants";
import AsyncStorage from "@react-native-async-storage/async-storage";
import { CachedFolder, getLocalCachedItems, saveCachedItems } from "./database";
...
async function cleanupOldSyncEntries(): Promise<void> {
  try {
    ...
    const syncKeys = allKeys.filter(
      (key) => key.startsWith(SYNC_PREFIX) && key !== CLEANUP_KEY,
    );
    ...
    const keysToRemove = entries
      .slice(0, entries.length - MAX_SYNC_ENTRIES)
      .map((e) => e.key);
  ...
  (async () => {
    try {
      ...
      if (apiItems.length === 0) {
        console.log("No content for ", folderPath);
        return;
      }
      ...
      // Find new items
      const newItems = apiItems.filter(
        (item) => !existingPaths.has(item.itemPath),
      );
 ...
 */
export async function forceSyncFolder(
  folderPath: string,
